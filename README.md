...
- **Live site (fresh public read-only check, 2026-09-26):** 3.10.412 (earlier today 3.10.405). Publicly served `frontend-cart.js` identifies as 412 **and still has the broken route join**. This does not prove the installed PHP bytes or who changed the site; no live cart mutations were made.
...
- **Tests:** 19 Python, 23 PHP contract assertions and 11 JS DOM checks. [GitHub Actions passed build plus real isolated WordPress 6.8/WooCommerce 10.2.0 and Chromium for 405 → 413](https://github.com/emeliijaddd-png/alookhor-update-publisher/actions/runs/36232484430): manual ZIP replacement **and** native Upgrader with mocked HTTP, corrupt-ZIP rejection, byte-for-byte installed-file comparison, guest cart/add/variation/quantity/removal. The latest CI also checks manual replacement from the separately pinned unsafe 412 ZIP; see the final run before installation. Neither the live theme/cache nor the actual configured download channel was exercised.
