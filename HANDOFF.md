# 📦 سند جامع انتقال — پروژهٔ آلوخور (به‌روز: v3.10.223)
...
- **افزونه:** `plugin/alookhor-control-center/` — نسخهٔ فعلی **3.10.223** (زنده و تأییدشده — PDP کاملاً به موکاپ جدید مالک بازطراحی شده؛ بصری کامل = 3.10.212 + پولیش ۳.10.214/۲۱۶/۲۱۷ (تومان، مخفی بودن وزنِ خالی، تراز تعداد، ارقام فارسی)؛ ۲۱۳/۲۱۵ فقط CI)
...
| **223** | **تبدیل دیزاین PDP به کد مرجع مالک** (زیپ convert-content): بوم تیرهٔ plum + پنل تخت + ریل عمودی تمبز ≥۱۱۰۱ + پالت gold/berry/mint + نستعلیق گوگل‌فونت؛ اسکوب تأییدشده: فقط بدنه — هدر/فوتر دست‌نخورده؛ مودال محصولات → جست‌وجوی واقعی (فاز B) | **زنده و سبز کامل** |
