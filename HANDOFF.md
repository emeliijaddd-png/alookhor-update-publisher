# 📦 سند جامع انتقال — پروژهٔ آلوخور (به‌روز: v3.10.224)
...
- **افزونه:** `plugin/alookhor-control-center/` — نسخهٔ فعلی **3.10.224** (زنده و تأییدشده — PDP کاملاً به موکاپ جدید مالک بازطراحی شده؛ بصری کامل = 3.10.212 + پولیش ۳.10.214/۲۱۶/۲۱۷ (تومان، مخفی بودن وزنِ خالی، تراز تعداد، ارقام فارسی)؛ ۲۱۳/۲۱۵ فقط CI)
...
| **217** | ارقام فارسی قیمت/تخفیف («۱٬۳۰۰ تومان» «٪۸۹») | زنده و سبز |
| **218/219** | **عاجل برنامهٔ SEO**: /item/* → 410؛ ابزار ادمین «🧹 پاک‌سازی SEO» (حذف نقشه‌های آلوده‌، ربات فیزیکی، دمورا به زبالت‌دان، محصولات بدون تصویر، پایش نفوذ سرور read-only) | زنده و سبز |
| **220** | فاز ۱ سئو تکنیکال (اسکیمای واحد، alt خودکار، لاگ ۴۰۴، پروب‌ها) — نتیجهٔ اسکرین‌شات مالک: .htaccess تمیز، بدون mu-plugin، uploads تمیز، blog_public باز، /%postname%/، Yoast غایب، ۱۳ محصول دمو، ۲ محصول بدون تصویر | زنده و سبز |
| **221** | رفع ۴۰۴ لوگوی کارت تبلیغی مگامنو (۲۰۲۶/۰۶→۲۰۲۶/۰۸) | زنده و سبز |
| **222** | frontend-responsive-v2.css — باند ۷۶۸–۱۱۰۰ + overflow-x:clip | زنده و سبز |
| **223** | تبدیل دیزاین PDP (همان مقادیر فعال؛ اثبات شد طرح فعلی از قبل از همان مرجع بود) + نستعلیق | زنده |
| **224** | **دلتای واقعی**: گرادیان ریشهٔ سلطنتی جلوی بوم جدید را می‌گرفت → canvas = app-bg مرجع (#170a20+۳ رادیال)، gold→#f7b32b/گلد-۲→#ffd37a، روبان گالری صورتی→گرادیان طلایی با چرخش −۳° (مثل React مالک) | **زنده و سبز کامل** |
...
- **v3.10.224 زنده، سبز و تأییدشده**
- **درس δ PDP**: کاربر «هیچ تغییری نمی‌بیند» درست می‌گفت از نظر بصری — طرح فعال (v212+) همان دیزاین مرجع بود؛ دلتاها فقط canvas/طلایی‌ها/روبان بودند (۲۲۴). قبل از تولید overrideها ابتدا CSS موجود را با توکن‌های مرجع diff کن.
- **پنل آپلود 8080** زنده است (/home/user/upload_server.py، پوشهٔ /home/user/uploads)؛ زیپ دیزاین: /home/user/target-design/
- **فاز B مودال**: ProductsModal مرجع → اتصال به جست‌وجوی واقعی ووکامرس (تأیید مالک: real-search)
- css-assets.zip = نسخهٔ قدیمی v2.7.13 استایل‌های ما؛ جدیدتر داریم → اعمال نشد تا حالا
- **نکتهٔ گیت**: قبل از هر release چک کن remote tip را fetch/rebase کنی — سندباکس گاهی به حالت برنچ‌پوینت برمی‌گردد، درختِ کار همیشه منبع حقیقت (resolve با --theirs است).
- **🚨 هک تاییدشده (Japanese Keyword Hack)**: `alookhor.ir/item/<عدد>` گاهی به `sales.losid.buzz` (فروشگاه هرزنامهٔ ژاپنی) ریدایرکت می‌شود؛ ~۱۷۶ نقشهٔ آلودهٔ `sitemap_*.xml` با امضای lastmod=2026-09-01؛ ریشه سمت سرور (.htaccess/فایل‌های هسته/mu-plugin) — راه‌حل: owner در پیشخوان آلوخور → «🧹 پاک‌سازی SEO» (پایش نفوذ + حذف نقشه‌های آلوده) + اسکن Wordfence + تغییر رمزها + GSC Removals پیشوند /item/. فروشگاه و صفحات واقعی سالم‌اند.
- **ممالزیت گِیت‌ها**: کُنج بازگشتِ سندباکس `.git` به حالت اولیه رخ داد؛ راه‌حل = fetch شات ریموت + rebase با تصمیم --theirs (درخت جاری همیشه منبع حقیقت است). — PDP جدید = موکاپ مالک (ران 33613266714 سبز کامل). هندسهٔ ۲۱۱ حفظ شده؛ بلوک‌های CSS قدیمی هیرو (۱۹۹–۲۱۱) مرده‌اند و بدرد پاک‌سازی می‌خورند.
