# 📦 سند جامع انتقال — پروژهٔ آلوخور (به‌روز: v3.10.213)
...
- **افزونه:** `plugin/alookhor-control-center/` — نسخهٔ فعلی **3.10.213** (زنده و تأییدشده — PDP کاملاً به موکاپ جدید مالک بازطراحی شده؛ بصری = 3.10.212، ۳.10.213 فقط اصلاح چک‌های CI)
...
| `scripts/wordpress_release_test.py` | تست پس از انتشار + `_pdp_browser_audit()`: سلکتورها از ۲۱۲ به `.alpm-*` تغییر کرده‌اند؛ چک‌های متنی به «قیمت محصول» وابسته‌اند — **اگر برچسب‌های مارک‌آپ PDP عوض شوند این دو چک را هم به‌روز کن** | |
...
| **213** | فقط اصلاح چک‌های product_endpoint/pdp_renders_authenticated به «قیمت محصول» | **زنده و کاملاً سبز (ok:true, failed:[])** |
...
- **v3.10.213 زنده، سبز و تأییدشده** — PDP جدید = موکاپ مالک (ران 33613266714 سبز کامل). هندسهٔ ۲۱۱ حفظ شده؛ بلوک‌های CSS قدیمی هیرو (۱۹۹–۲۱۱) مرده‌اند و بدرد پاک‌سازی می‌خورند.
