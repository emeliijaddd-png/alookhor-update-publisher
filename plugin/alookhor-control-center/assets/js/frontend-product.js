...
function gallery(){
 const stage=$('#alpMain');
 const frame=$('[data-stage]') || (stage ? stage.closest('.gallery-frame') : null);
 ...
 const tick=()=>{
   ...
   if(frame && frame.matches && frame.matches(':hover')) return; // don't auto while hovering for zoom
   // if user is interacting with thumbs, pause is already set
   show(idx+1);
 };
 const startAuto=()=>{
 ...
 // v3.10.355 — ZOOM on hover (mouse) + auto-rotate every 4s
 if(frame && stage){
   // zoom
   let isZooming=false;
   const onMove=(e)=>{
   ...
   frame.addEventListener('mouseenter',()=>{
     if(window.matchMedia && window.matchMedia('(max-width:1024px)').matches) return; // no zoom on touch
     isZooming=true;
     frame.classList.add('is-zooming');
     stage.style.transform='scale(2)';
     stage.style.transition='transform .28s ease, transform-origin .08s linear';
     pauseAuto();
   });
   ...
   frame.addEventListener('mouseleave',()=>{
     ...
     autoPausedUntil=Date.now()+1000;
   });
   // touch devices: double tap to zoom? keep simple
 }

 startAuto();
 // pause when tab hidden
 document.addEventListener('visibilitychange',()=>{ if(document.hidden) stopAuto(); else startAuto(); });
 // pause on interaction with frame
 if(frame){
...
/* v3.10.355: quantity + cart URL builder - preserves variation */
let _alpQty=1;
...
function weights(){
 ...
 // v3.10.355: auto-init first weight on load so add-to-cart works without extra click
