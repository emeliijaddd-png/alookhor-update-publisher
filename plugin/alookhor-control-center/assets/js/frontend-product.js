...
function quantity(){
 ...
 $$('[data-q]').forEach(b=>b.addEventListener('click',()=>{
  const raw=String(b.dataset.q||'').trim(); const dir=raw==='+'?1:(raw==='-'||raw==='\u2212')?-1:(parseInt(raw,10)||0); /* v3.10.267: markup uses data-q="+"/"-" */
...
/* ===== v3.10.267 — MOBILE (<=1279px) ONLY: usage / FAQ / reviews are always-visible standalone sections on phones
...
/* ===== v3.10.267 — make the existing "مشاهده بیشتر" button work: markup uses data-desc-toggle while descExpand()
