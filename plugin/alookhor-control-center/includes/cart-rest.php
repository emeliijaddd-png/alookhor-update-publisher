...
function alookhor_cc_cart_payload() {
    ...
    foreach (WC()->cart->get_cart() as $key => $item) {
        ...
        $items[] = [
            ...
            // Read the actual discounted line from WooCommerce, not price × qty.
...
function alookhor_cc_cart_persist() {
    WC()->cart->calculate_totals();
    WC()->cart->set_session();
    // Some Woo/theme integrations do not set the guest cookie until the next page.
    // Send it on the mutation response so the next REST GET reads this same cart.
...
add_action('wp_loaded', function () {
    ...
    alookhor_cc_cart_persist(); // persist a guest's selected quantity before redirect
...
add_action('rest_api_init', function () {
    ...
    register_rest_route('alookhor-cart/v4', '/cart/add', [
        ...
        'callback' => function (WP_REST_Request $request) {
            ...
            if (!$parent || (!$parent->is_type('simple') && !$parent->is_type('variable'))) {
                return new WP_Error('product_unavailable', 'این محصول در حال حاضر قابل خرید نیست.', ['status' => 400]);
            }
            if ($parent->is_type('variable')) {
                if (!$variation_id) {
                    return new WP_Error('variation_required', 'لطفاً وزن و بسته‌بندی محصول را انتخاب کنید.', ['status' => 400]);
                }
                $product = wc_get_product($variation_id);
                if (!$product || !$product->is_type('variation') || (int) $product->get_parent_id() !== $product_id) {
                    return new WP_Error('invalid_variation', 'گزینه انتخاب‌شده متعلق به این محصول نیست.', ['status' => 400]);
                }
                foreach ($product->get_variation_attributes() as $attribute => $value) {
                    if ($value !== '' && (!isset($variation[$attribute]) || $variation[$attribute] !== $value)) {
                        return new WP_Error('invalid_variation', 'گزینه‌های محصول با وزن انتخابی مطابقت ندارند.', ['status' => 400]);
                    }
                }
            } else {
                if ($variation_id) return new WP_Error('invalid_variation', 'این محصول گزینهٔ متغیر ندارد.', ['status' => 400]);
                $product = $parent;
            }
            ...
            if (!$added_key) {
                ...
                $message = !empty($notices[0]['notice']) ? wp_strip_all_tags($notices[0]['notice']) : 'افزودن محصول به سبد خرید انجام نشد.';
                return new WP_Error('add_failed', $message, ['status' => 400]);
            }
