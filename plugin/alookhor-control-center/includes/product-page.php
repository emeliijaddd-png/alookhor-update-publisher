...
function alookhor_cc_pdp_card($pid){
 ...
 $name=$p->get_name();
 // A variable parent needs a chosen variation: never label a product-page link as an instant add.
 $select_options = $p->is_type('variable');
 $action_url = $select_options ? get_permalink($p->get_id()) : $p->add_to_cart_url();
 $action_label = $select_options ? 'انتخاب وزن و افزودن ' . $name : 'افزودن ' . $name;
 $hint = $select_options ? '<small class="suggested-choose">برای افزودن، وزن را انتخاب کنید</small>' : '';
 $out='<article class="suggested-product group"><a href="'.esc_url(get_permalink($p->get_id())).'" class="suggested-image" aria-label="مشاهده '.esc_attr($name).'"><img src="'.esc_url($u).'" alt="'.esc_attr($name).'" loading="lazy"><span class="suggested-heart">'.alookhor_cc_pdp_icon('heart').'</span><span class="product-tag">پیشنهاد الخور</span></a><div class="suggested-body"><h3>'.esc_html($name).'</h3><div class="suggested-footer"><span class="suggested-price">'.$price_clean.'</span><a href="'.esc_url($action_url).'" aria-label="'.esc_attr($action_label).'" title="'.esc_attr($action_label).'" class="suggested-cart">'.alookhor_cc_pdp_icon('cart').'</a></div>'.$hint.'</div></article>';
 return $out;
}
}
