...
function alookhor_cc_cart_suggestions_html($recs, $wish_ids = array()) {
    ...
    foreach ((array) $recs as $p) {
        ...
        ?>
<article class="sg-card" data-base-price="<?php echo esc_attr($price); ?>">
  <span class="sg-img">
  ...
  <?php if (($p['type'] ?? '') === 'variable' && !empty($p['options'])): ?>
  <label class="sg-choose">وزن / بسته‌بندی
    <select class="sg-variant" aria-label="انتخاب گزینه برای <?php echo esc_attr($name); ?>">
      <option value="">انتخاب کنید</option>
      <?php foreach ($p['options'] as $option): ?>
      <option value="<?php echo esc_attr($option['variation_id']); ?>" data-attributes="<?php echo esc_attr(wp_json_encode($option['attributes'], JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT)); ?>" data-price="<?php echo esc_attr($option['price']); ?>"><?php echo esc_html($option['label'] . ' — ' . alookhor_cc_cart_fa($option['price']) . ' تومان'); ?></option>
      <?php endforeach; ?>
    </select>
  </label>
  <?php endif; ?>
  <?php if (($p['type'] ?? '') === 'variable' && empty($p['options'])): ?>
    <a class="sg-add" href="<?php echo esc_url($p['permalink']); ?>">انتخاب گزینه‌ها در صفحه محصول</a>
  <?php else: ?>
  ...
  <?php endif; ?>
  <?php if (!empty($p['more_options'])): ?><a class="sg-more" href="<?php echo esc_url($p['permalink']); ?>">گزینه‌های بیشتر</a><?php endif; ?>
</article>
        <?php
    }
    return ob_get_clean();
...
function alookhor_cc_cart_variation_options($product, $limit = 30) {
    if (!function_exists('wc_get_product') || !$product->is_type('variable')) return array();
    $options = array();
    foreach ($product->get_children() as $child_id) {
        $variant = wc_get_product($child_id);
        if (!($variant instanceof WC_Product_Variation) || !$variant->is_purchasable() || !$variant->is_in_stock()) continue;
        $attrs = $variant->get_variation_attributes();
        // A wildcard variation needs a second attribute choice; do not sell an arbitrary option.
        if (!$attrs || in_array('', $attrs, true)) continue;
        $labels = array();
        foreach ($attrs as $attribute => $value) {
            $taxonomy = str_replace('attribute_', '', $attribute);
            $term = function_exists('get_term_by') ? get_term_by('slug', $value, $taxonomy) : false;
            $name = $term && !is_wp_error($term) ? $term->name : $value;
            $labels[] = wc_attribute_label($taxonomy, $product) . ': ' . $name;
        }
        $options[] = array(
            'variation_id' => (int) $variant->get_id(),
            'attributes' => $attrs,
            'label' => implode(' / ', $labels),
            'price' => (float) wc_get_price_to_display($variant),
        );
...
function alookhor_cc_cart_recommendations($limit = 4) {
    ...
    foreach ($products as $product) {
        ...
        if (!$product || !is_object($product) || !$product->is_in_stock()) continue;
        if (!$product->is_type('simple') && !$product->is_type('variable')) continue;
        if ($product->is_type('simple') && !$product->is_purchasable()) continue;
        $is_variable = $product->is_type('variable');
        $options = $is_variable ? alookhor_cc_cart_variation_options($product) : array();
        ...
        $out[] = array(
            ...
            'options' => $options,
            'more_options' => $is_variable && count($product->get_children()) > count($options),
...
function alookhor_cc_cart_markup(){
    ...
    $lines  = count($items);
    $units  = $initial ? (int) ($initial['count'] ?? array_sum(array_column($items, 'quantity'))) : 0;
    ...
    <div class="ac-grid">
      <section class="ac-products" aria-labelledby="ac-products-title">
        <div class="ac-products-head">
          <strong id="ac-products-title">محصولات شما <span class="cart-items-count">(<?php echo $initial === null ? '…' : esc_html(alookhor_cc_cart_fp($lines) . ' نوع، ' . alookhor_cc_cart_fp($units) . ' عدد'); ?>)</span></strong>
...
<?php if ($initial !== null) echo '<script id="alookhor-cart-initial" type="application/json">' . wp_json_encode($initial, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) . '</script>'; ?>
