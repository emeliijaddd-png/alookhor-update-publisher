## 3.10.375 — فیکس تسویه حساب زیر فوتر
- FIX: کاربر گفت رفته زیر فوتر باید بالای فوتر بیاد؛ علت رندر چندباره checkout (template_include + the_content + wc_get_template) که نسخه دوم زیر فوتر می‌افتاد. فیکس: چک GLOBALS در ابتدای markup و در wc_get_template/locate_template
- Version bump 3.10.374->3.10.375
...
- Version bump 3.10.371->3.10.372

## 3.10.371 — صفحه تسویه حساب لوکس - طبق تصویر
- NEW: صفحه تسویه حساب لوکس طبق تصویر: استپس بار 3 مرحله‌ای طلایی، فرم اطلاعات مشتری (نام، موبایل، آدرس)، روش ارسال (پست پیشتاز رایگان / تیپاکس)، روش پرداخت (آنلاین / در محل)، خلاصه سفارش واقعی از WC()->cart، کد تخفیف، 4 ویژگی پایین
- FIX: حذف قالب‌های قدیمی المنتور/وودمارت برای checkout با display:none، override template_include و wc_get_template به لوکس
- REAL: دیتای واقعی از WC()->cart و WC()->customer، شورت‌کدهای [alookhor_checkout] و ...
- Version bump 3.10.370->3.10.371
...
- KEEP: هدر و فوتر حفظ
- Version bump 3.10.368->3.10.369

## 3.10.368 — سبد خرید: فیکس المنتور - شورت‌کدها خالی بود
- FIX: در تصویر المنتور [alookhor_cart_hero] خالی بود؛ علت: CSS فقط در is_cart لود می‌شد و WC()->cart در پیش‌نمایش null بود
- FIX: همه شورت‌کدها با inline CSS داخلی و دیتای fallback نمایشی بازنویسی شد + CSS سبد در همه صفحات المنتور لود + [alookhor_cart_test] اضافه شد
- Version bump 3.10.367->3.10.368
...
- Version bump 3.10.362->3.10.363
