...
## 3.10.241 — CART + QTY PILL PER PHOTO1 + PRICE PER PHOTO2 + FEATURES 4 PER PHOTO3 + GUARANTEES 4 UNDER IMAGE PER PHOTO4
- User 4 photos 2026-09-03 13:30: (1) cart+qty+icons, (2) price, (3) features 4, (4) guarantees 4 under image.
- Step-by-step:
  1) Cart+Qty per photo1: quantity-row flex justify-between label تعداد: right + control pill rounded-full bg plum-950/70 border white/10 p-1 with - 1 + order (minus left, plus right), id alpQty single, add-button gold full width + 3 icons share/compare/wishlist each 44px dark glass, shipping line green dot موجود در انبار left + ارسال از ... right.
  2) Price per photo2: price-box p-4, label قیمت محصول: + discount badge rounded-full plum-800 border berry/30, original line-through 11px lav, discounted 20-24px gold luminous, layout justify-between.
  3) Features per photo3: product-features xl:grid-cols-4 (was 2) = 4 in row desktop, 2 mobile, cards dark glass.
  4) Guarantees per photo4: guarantees-glass-slot under gallery (already) now 4 items grid-cols-2 sm:grid-cols-4, items: ضمانت اصالت, بسته‌بندی بهداشتی, 7 روز بازگشت, پشتیبانی همیشگی with icons shield-check/box/return/headset, rounded-xl bg plum-950/60.
- Kept: breadcrumb 140px separate frame, weight 3 cols with price, panel 0.95fr image 1.22fr, images fill frame, MOCKUP PDP marker.
