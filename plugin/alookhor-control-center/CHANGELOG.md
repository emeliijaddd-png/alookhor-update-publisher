## 3.10.318 — PHONE RAILS = SLIDERS — «محصولات پیشنهادی» و «گزیده‌ای از بهترین آلو بخارایی» روی موبایل اسلایدی شدند
- درخواست مالک: در موبایل این دو بخش باید اسلایدی باشند (نسخه ۳.۱۰.۳۱۱ آن‌ها را گرید ۲ ستونه کرده بود).
- CSS فقط داخل @media(max-width:640px): ریل افقی با اسکرول لمسی، هر کارت ۷۰٪ عرض + پیش‌نمایش کارت بعدی، scroll-snap، فلش‌های گرد دو طرف دوباره نمایان.
- JS: فلش‌ها بر اساس جهت نوشتار کار می‌کنند (در RTL فلش چپ = کارت بعدی، فلش راست = قبلی؛ قبلاً برعکس بود و روی دسکتاپ هم چون همه کارت‌ها جا می‌شوند اسکرولی وجود ندارد → بدون تغییر رفتار). گام حرکت در موبایل = عرض یک کارت + فاصله.
- DESKTOP LOCK: مقایسه پیکسلی ۱۴۴۰×۳۳۶۹ با ۳.۱۰.۳۱۷ → ۰ پیکسل تفاوت؛ تبلت ۷۶۸/۱۰۲۴ گرید ۳ ستونه بدون تغییر؛ عرض اسکرول صفحه در ۳۶۰/۳۹۰/۴۳۰ = عرض صفحه.

## 3.10.317 — PHONE BUY BOX RE-LOCK — جعبه خرید موبایل دوباره مطابق ماکاپ ۱ (فقط ≤640px)
- ROOT CAUSE: بلوک «OWNER FINAL LAYOUT» نسخه ۳.۱۰.۳۱۲ بدون media query و با !important در انتهای فایل CSS نشسته بود و روی موبایل قوانین ۳.۱۰.۳۱۱ را می‌شکست: پیل تعداد به یک ردیف کامل بالای دکمه تبدیل شده بود، ۳ آیکن (علاقه‌مندی/مقایسه/اشتراک) در ردیف سوم ظاهر می‌شدند و خط «موجود در انبار» جابه‌جا شده بود.
- FIX: بلوک جدید فقط داخل @media(max-width:640px) و بعد از بلوک ۳۱۲ اضافه شد: پیل تعداد راست + دکمه طلایی «افزودن به سبد خرید» چپ در یک خط (۴۴px)، ردیف آیکن‌ها مخفی، «ارسال از ۱ روز کاری آینده» راست و «● موجود در انبار» چپ — دقیقاً ماکاپ ۱.
- DESKTOP LOCK: اسکرین‌شات ۱۴۴۰×۳۳۶۹ نسخه قبلی و نسخه جدید پیکسل‌به‌پیکسل مقایسه شد → ۰ پیکسل تفاوت (چیدمان جعبه خرید ۳۱۲ روی دسکتاپ دست‌نخورده). تبلت ۷۶۸/۱۰۲۴ هم بدون تغییر. عرض اسکرول ۳۶۰/۳۹۰/۴۳۰ = عرض صفحه (بدون سرریز).
- JS: با عوض کردن چیپ وزن، تعداد انتخاب‌شده در لینک افزودن به سبد (دکمه اصلی + نوار چسبان) صفر نمی‌شود (قبلاً quantity حذف می‌شد و ۱ عدد به سبد می‌رفت).
