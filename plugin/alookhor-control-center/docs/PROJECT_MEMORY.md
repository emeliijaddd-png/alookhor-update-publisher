...
- **Release آماده انتشار اتمی:** `3.10.19` (Top Bar و منوی اصلی دو‌ردیفه مطابق مرجع؛ هنوز Tag نخورده و روی Production نیست)
- **تصمیم‌های نشست ۲۰۶-۰۸-۲۱:** بلوک بنفش باقی‌مانده «DEEP PURPLE GLASS» از `frontend-header.css` حذف شد (Burgundy/Gold تنها پالت هدر؛ با assert در CI قفل شد). مگامنو به‌عنوان بعدی Scope مدیریت‌شده و به کاندیدای هنوز‌نشرنشده `3.10.19` ادغام می‌شود (تصمیم مالک: یک انتشار).
