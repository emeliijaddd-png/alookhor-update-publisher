...
// FORCE: Override Elementor checkout widget - prevents white old checkout
add_filter('elementor/widget/render_content',function($content,$widget){
 if(function_exists('is_checkout')&&is_checkout()&&!is_wc_endpoint_url('order-received')&&!is_admin()){
   if(!empty($GLOBALS['alookhor_cc_checkout_rendered'])){
     if(is_object($widget)){
       $name=$widget->get_name();
       if(strpos($name,'checkout')!==false || strpos($name,'billing')!==false || strpos($name,'shipping')!==false) return '';
     }
     if(strpos($content,'جزئیات صورتحساب')!==false || strpos($content,'صورتحساب')!==false) return '';
     return $content;
   }
   if(is_object($widget)){
     $name=$widget->get_name();
     $checkout_widgets=['woocommerce-checkout','checkout','wd_checkout','wd_woocommerce_checkout','billing','shipping','order_review','checkout_form'];
     if(in_array($name,$checkout_widgets) || strpos($name,'checkout')!==false){
       if(function_exists('alookhor_cc_checkout_markup')){
         $m=alookhor_cc_checkout_markup();
         if($m) return $m;
       }
   ...
   if(strpos($content,'جزئیات صورتحساب')!==false || strpos($content,'صورتحساب شما')!==false){
     if(function_exists('alookhor_cc_checkout_markup')){
       $m=alookhor_cc_checkout_markup();
       if($m) return $m;
     }
